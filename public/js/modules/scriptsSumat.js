...
{
	var formSumat = $('.add-sumat');
	formSumat.on('submit', function()
	{
		$.ajax({
			type: formSumat.attr('method'),
			url: formSumat.attr('action'),
			data: formSumat.serialize(),
...
function ListSumat()
{
	var divSumat = $('#listSumat');
	var spanTotal = $('#total');
	var route = 'http://notes.dev/sumat';
	$('#listSumat').empty();
	$('#total').empty();
	$.get(route, function(respuesta)
	{
		...
		$(respuesta).each(function(key, value)
		{
			...
			if(value.estatus == 'pago')
			{
				...
				panel += '<button value='+ value.id +' onclick="ShowSumat(this);" data-toggle="modal" data-target="#myModal2" class="btn btn-warning"><i class="fa fa-pencil-square fa-fw"></i></button> ';
				panel += ' <button value='+ value.id +' onclick="ShowSumatDelete(this);" data-toggle="modal" data-target="#myModal3" class="btn btn-danger"><i class="fa fa-trash fa-fw"></i></button>';		
				panel += '</li>';
				divSumat.append(panel);
 ...
 *	*************** EDIT ***************
 */
function ShowSumat(boton)
{
	var route = 'http://notes.dev/sumats/'+ boton.value +'/edit';
	$.get(route, function(respuesta)
	{
...
$('#edit-sumat').click(function()
{
	...
	var route = 'http://notes.dev/sumats/'+id+'';
	var token = $('#token').val();
 ...
 *	*************** DELETE ***************
 */
function ShowSumatDelete(boton)
{
	var route = 'http://notes.dev/sumats/'+ boton.value + '/edit';
	$.get(route, function(respuesta)
	{
