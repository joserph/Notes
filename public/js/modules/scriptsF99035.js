...
{
	var formF99035 = $('.add-f99035');
	formF99035.on('submit', function()
	{
		$.ajax({
			type: formF99035.attr('method'),
			url: formF99035.attr('action'),
			data: formF99035.serialize(),
...
function ListF99035()
{
	var divF99035 = $('#listF99035');
	var spanTotal = $('#total');
	var route = 'http://notes.dev/iva99035';
	$('#listF99035').empty();
	$('#total').empty();
	$.get(route, function(respuesta)
	{
		...
		$(respuesta).each(function(key, value)
		{
			...
			if(value.estatus == 'pago')
			{
				...
				panel += 'Se pago '+ pago +' ';					
				panel += '</span>';
				panel += '<h4 class="text-uppercase">Periodo del '+ value.periodo +'</h4>';
				panel += '<hr />';
				panel += '<p>Pagado por <em><b>'+ value.pagado_por +'</b></em></p>';
				panel += '<p>Monto: <kbd>'+ formatNumber.new(value.monto) +'</kbd></p>';
				panel += '<button value='+ value.id +' onclick="ShowF99035(this);" data-toggle="modal" data-target="#myModal2" class="btn btn-warning"><i class="fa fa-pencil-square fa-fw"></i></button> ';
				panel += ' <button value='+ value.id +' onclick="ShowF99035Delete(this);" data-toggle="modal" data-target="#myModal3" class="btn btn-danger"><i class="fa fa-trash fa-fw"></i></button>';		
				panel += '</li>';
				divF99035.append(panel);
			}else if(value.estatus == 'vencido')
			{
				...
				panel += '<h4 class="text-uppercase">'+ value.periodo +'</h4>';
				panel += '<hr />';
				panel += '<button value='+ value.id +' onclick="ShowF99035(this);" data-toggle="modal" data-target="#myModal2" class="btn btn-warning"><i class="fa fa-pencil-square fa-fw"></i></button> ';
				panel += ' <button value='+ value.id +' onclick="ShowF99035Delete(this);" data-toggle="modal" data-target="#myModal3" class="btn btn-danger"><i class="fa fa-trash fa-fw"></i></button>';		
				panel += '</li>';
 ...
 *	*************** EDIT ***************
 */
function ShowF99035(boton)
{
	var route = 'http://notes.dev/iva99035s/'+ boton.value +'/edit';
	$.get(route, function(respuesta)
	{
...
$('#edit-f99035').click(function()
{
	...
	var route = 'http://notes.dev/iva99035s/'+id+'';
	var token = $('#token').val();
 ...
 *	*************** DELETE ***************
 */
function ShowF99035Delete(boton)
{
	var route = 'http://notes.dev/iva99035s/'+ boton.value + '/edit';
	$.get(route, function(respuesta)
	{
