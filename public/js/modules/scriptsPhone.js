...
{
	var formPhone = $('.add-phone');
	formPhone.on('submit', function()
	{
		$.ajax({
...
function ListPhones()
{
	var divPhones = $('#listPhones');
	var route = 'http://notes.dev/phone';
	$('#listPhones').empty();
	$.get(route, function(respuesta)
	{
		$(respuesta).each(function(key, value)
		{
			...
			if(value.estatus == 'pago')
			{
				...
				panel += '<button value='+ value.id +' onclick="ShowPhone(this);" data-toggle="modal" data-target="#myModal2" class="btn btn-warning"><i class="fa fa-pencil-square fa-fw"></i></button> ';
				panel += ' <button value='+ value.id +' onclick="ShowPhoneDelete(this);" data-toggle="modal" data-target="#myModal3" class="btn btn-danger"><i class="fa fa-trash fa-fw"></i></button>';		
				panel += '</li>';
				divPhones.append(panel);
 ...
 *	*************** EDIT ***************
 */
function ShowPhone(boton)
{
	var route = 'http://notes.dev/phones/'+ boton.value +'/edit';
	$.get(route, function(respuesta)
	{
...
$('#edit-phone').click(function()
{
	...
	var route = 'http://notes.dev/phones/'+id+'';
	var token = $('#token').val();
 ...
 *	*************** DELETE ***************
 */
function ShowPhoneDelete(boton)
{
	var route = 'http://notes.dev/phones/'+ boton.value + '/edit';
	$.get(route, function(respuesta)
	{
